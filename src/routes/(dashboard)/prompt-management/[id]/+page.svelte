	...
	async function restoreVersion(version: number) {
		...
		try {
			const response = await fetch(
				`/api/prompts/${prompt.id}/versions/${version}/restore`,
				{
					method: "POST",
				},
			);
		...
		<div class="flex items-start justify-between">
			<div>
				<h1 class="text-2xl font-bold text-gray-900">
					{typeLabel} - Version History
				</h1>
				<p class="text-gray-600 mt-1 text-sm">{prompt.name}</p>
			</div>
			<div class="flex items-center gap-3">
				<span
					class="px-3 py-1 bg-indigo-100 text-indigo-700 text-sm font-medium rounded"
				>
	...
	<div class="mb-8 bg-white border-2 border-indigo-200 rounded-lg p-6">
		...
		<div class="mb-4">
			...
			<div class="bg-gray-50 border border-gray-200 rounded p-4">
				<p
					class="text-xs font-mono text-gray-700 whitespace-pre-wrap break-words"
				>
		...
		{#if versions.length === 0}
			...
			>
				<p class="text-gray-600">
					No previous versions yet. Edit this prompt to create
					versions.
				</p>
			</div>
		{:else}
					...
					>
						<div class="flex items-start justify-between mb-3">
							<div class="flex-1">
								...
								<div
									class="flex items-center gap-4 text-xs text-gray-500 mb-2"
								>
									<span>{formatDate(version.created_at)}</span
									>
									<span>•</span>
									<span
										>{version.content.length} characters</span
									>
								</div>
						...
						{#if expandedVersions.has(version.version)}
							<div
								class="mt-3 bg-gray-50 border border-gray-200 rounded p-4"
							>
								<p
									class="text-xs font-mono text-gray-700 whitespace-pre-wrap break-words"
								>
		...
		>
			...
			<div class="border-b border-gray-200 px-6 py-4">
				...
				<p class="text-sm text-gray-600 mt-1">
					{formatDate(viewingContent.created_at)}
				</p>
			</div>
			...
			<!-- Modal Footer -->
			<div
				class="border-t border-gray-200 px-6 py-4 flex justify-end gap-2"
			>
				<button
					on:click={closeContentView}
