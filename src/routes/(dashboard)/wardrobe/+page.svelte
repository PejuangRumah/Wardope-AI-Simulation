<script lang="ts">
	...
	import EditWardrobeItemModal from "$lib/components/EditWardrobeItemModal.svelte";
	import type { WardrobeItem } from "$lib/types/wardrobe";

	export let data;

	let showAddModal = false;
	let editingItem: WardrobeItem | null = null;
	...
	function handleModalSuccess() {
		showAddModal = false;
		editingItem = null;
		// Reload page to show new/updated item
		location.reload();
	}
		...
		<div class="flex flex-col items-center justify-center min-h-[60vh]">
			<div class="text-center max-w-md">
				...
				<p class="text-gray-600 mb-6">
					You need to configure AI prompts before using the wardrobe
					features. This only takes a moment.
				</p>
				<a
			...
			>
				<div>
					...
					<p class="text-gray-600 mt-1">
						{data.total}
						{data.total === 1 ? "item" : "items"} in your collection
					</p>
				</div>
				<button
					on:click={() => (showAddModal = true)}
					class="w-full sm:w-auto px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 font-medium shadow-md"
				>
					<Plus class="w-5 h-5" />
					Add Item
				</button>
