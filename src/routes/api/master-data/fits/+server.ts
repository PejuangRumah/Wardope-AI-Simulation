...
// GET - List all fits with category names
export const GET: RequestHandler = async ({ locals: { supabase } }) => {
	try {
		...
		return json({ fits: data });
	} catch (error) {
		console.error('Error fetching fits:', error);
		return json({ error: 'Failed to fetch fits' }, { status: 500 });
	}
};
...
export const POST: RequestHandler = async ({ request, locals: { supabase } }) => {
	try {
		...
		return json({ fit: data }, { status: 201 });
	} catch (error) {
		console.error('Error creating fit:', error);
		return json({ error: 'Failed to create fit' }, { status: 500 });
	}
};
